...
def show_pcl(pcl):
    ...
    def key_callback(obj):
        obj.close()
        return False
    
    obj = o3d.visualization.VisualizerWithKeyCallback()
    obj.create_window()
    ...
    obj.add_geometry(pcd)

    # step 5 : visualize point cloud and keep window open until right-arrow is pressed (key-code 262)
    obj.register_key_callback(262, key_callback)
    obj.run()
    obj.destroy_window()
    #######
    ####### ID_S1_EX2 END #######     
...
def show_range_image(frame, lidar_name):
    ...
    # Extract the number of columns in range image to be considered
    coverage_from_center = 90 # in degrees
    n_cols = int(range_image.shape[1] / (360 // coverage_from_center))
    center = int(range_image.shape[1] / 2)
   
    # step 4 : map the range channel onto an 8-bit scale and make sure that the full range of values is appropriately considered
    range_channel = (range_channel / (np.amax(range_channel) - np.amin(range_channel))) * 255
    range_channel = range_channel.astype(np.uint8)
    range_channel = range_channel[:, center - n_cols : center + n_cols]

    # step 5 : map the intensity channel onto an 8-bit scale and normalize with the difference between the 1- and 99-percentile to mitigate the influence of outliers
    intensity_channel = (np.amax(intensity_channel)/2) * (intensity_channel / (np.amax(intensity_channel) - np.amin(intensity_channel))) * 255
    intensity_channel = intensity_channel.astype(np.uint8)
    intensity_channel = intensity_channel[:, center - n_cols : center + n_cols]
