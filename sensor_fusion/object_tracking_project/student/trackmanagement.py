...
class Trackmanagement:
    ...
    def manage_tracks(self, unassigned_tracks, unassigned_meas, meas_list):  
        ...
        for track in self.track_list:
            p_val = (track.P[0, 0] > params.max_P) or (track.P[1, 1] > params.max_P)
            delete_track = False
            if (track.score < params.delete_threshold_min):
                delete_track = True
            elif (((track.state == 'initialized') or (track.state == 'tentative')) and p_val):
                delete_track = True
            elif((track.state == 'confirmed') and ((track.score < params.delete_threshold) or p_val)):
                delete_track = True
            if (delete_track):
    ...
    def handle_updated_track(self, track):      
        ...
        if (params.delete_threshold < track.score < params.confirmed_threshold):
