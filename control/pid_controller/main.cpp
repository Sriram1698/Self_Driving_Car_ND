...
int get_nearest_point_on_traj(double x, double y, const vector<double>& x_traj, const vector<double>& y_traj)
{
  double min_distance = INFINITY;
  if (x_traj.size() != y_traj.size()) 
  {
    return 0;
  }
  ...
  double dist = 0;
  int min_index = 0;
  for (uint32_t i = 0; i < x_traj.size(); i++)
  {
      dist = sqrt(pow(x_traj[i] - x, 2) + pow(y_traj[i] - y, 2));
      if ((dist < min_distance) && (x_traj[i] > x))
      {
        min_distance = dist;
        min_index = i;
      }
...
int main ()
{
  ...
  **/
  PID pid_steer = PID();
  // pid_steer.Init(0.01, 0.0000, 0.00, 1.2, -1.2); // oscillation with reference to 0 error (used on "p" gain)
  // pid_steer.Init(0.01, 0.0000, 0.05, 1.2, -1.2); // introduced a small "d" gain, the change in error reduced a little
  // pid_steer.Init(0.01, 0.0001, 0.05, 1.2, -1.2); // introduced a small "i" gain, the steady-state error started to decrease
  // pid_steer.Init(0.02, 0.0001, 0.05, 1.2, -1.2); // increaed the "p" gain, steer corrections are faster comparitively
  // pid_steer.Init(0.02, 0.0001, 0.1, 1.2, -1.2);
  // pid_steer.Init(0.02, 0.0001, 0.2, 1.2, -1.2);
  pid_steer.Init(0.028, 0.0001, 0.3, 1.2, -1.2); // finalized gains, as it is giving good control over the steering
  ...
  **/
  ...
  // pid_throttle.Init(0.1, 0.00, 0.0, 1, -1); // used only "p" gain, oscaillation behavior
  // pid_throttle.Init(0.1, 0.00, 0.02, 1, -1); // introduced "d" gain, reached the target vel quickly but overshooting
  // pid_throttle.Init(0.1, 0.01, 0.02, 1, -1); // introduced "i" gain, the error accumulation help to reached desired vel over the time
  pid_throttle.Init(0.1, 0.01, 0.3, 1, -1); // the finalized gains, as it follows the velocity profile closely
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          const int nearest_point_on_traj = get_nearest_point_on_traj(x_position, y_position, x_points, y_points);
          /**
          ...
          **/
          // error_steer = yaw - angle_between_points( x_position, y_position, x_points[nearest_point_on_traj], y_points[nearest_point_on_traj]);
          ...
          **/
          ...
          // error_throttle = v_points[nearest_point_on_traj] - velocity;
